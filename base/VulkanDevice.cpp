...
{	
	...
	* 在设备上创建一个缓冲区
	*
	* @param usageFlags缓冲区的使用标志位掩码(即索引，顶点，统一缓冲区)
	* @param memoryPropertyFlags该缓冲区的内存属性(即设备本地，主机可见，一致)
	* @param size缓冲区的大小，单位为byes
	* @param buffer指向函数获取的缓冲区句柄的指针
	* @param memory函数获取的内存句柄的指针
	* @param data指针，指向创建后应该复制到缓冲区的数据(可选，如果不设置，则不复制数据)
	*
	* @返回VK_SUCCESS如果缓冲区句柄和内存已经创建和(可选传递)数据已经复制
	*/
	...
	* @param usageFlags缓冲区的使用标志位掩码(即索引，顶点，统一缓冲区)
	* @param memoryPropertyFlags该缓冲区的内存属性(即设备本地，主机可见，一致)
	* @param buffer指向vk::Vulkan缓冲区对象的指针
	* @param size缓冲区大小(以字节为单位)
	* @param data指针，指向创建后应该复制到缓冲区的数据(可选，如果不设置，则不复制数据)
	*
	* @返回VK_SUCCESS如果缓冲区句柄和内存已经创建和(可选传递)数据已经复制
	*/
